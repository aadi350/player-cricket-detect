...
def get_classification(img) -> str:
	with open('/home/aadidev/projects/player-cricket-detect/ImageNetLabels.txt') as classesfile:
		classeslist = classesfile.readlines()
		classeslist = [class_.replace('\n', '') for class_ in classeslist if class_ != '\n']
		classifier = _load_classifier([None, 192, 192, 3])
		res = np.argmax(classifier(img), axis=1)[0]
		print(res)
		return classeslist[res]


def _load_classifier(shape) -> object:
	m = tf.keras.Sequential([
		hub.KerasLayer("https://tfhub.dev/google/imagenet/mobilenet_v2_075_192/classification/4")
	])
	m.build(shape)  # Batch input shape.
	return m
...
plt.suptitle('Plot Showing Accuracy Using MobileNet Features')
plt.imsave()
plt.show()
